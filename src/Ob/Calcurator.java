...
//setOper(int left, int write) 메서드를 만든 이유는 이 클래스의 인스턴스 변수인 left와 write를 설정하기 위해서입니다. 이 메서드를 사용하여 외부에서 left와 write 값을 설정할 수 있습니다. 이를테면, Calcurator 객체를 생성한 후에 setOper 메서드를 호출하여 left와 write 값을 설정하면, 해당 값들이 메서드 내부에서 인스턴스 변수에 할당됩니다.
//
...
public class Calcurator {
    ...
    public void setOper(int left, int write ){

        this.left=left; //밑에 선언 타고 올라온 위의 left/write 바로 이거라는 뜻
        this.write=write;

    }
    public void sum(){
        System.out.println(this.left+this.write);
    }

    public void avg(){
        System.out.println((this.left+this.write)/2);
    }
    ...
    public static void main(String[] args) {
        ...
        // 인스턴스의 소유가 아닌 변수가 있을 수 있나? 있다. 클래스의 소유인 변수가 있다. 다른 특성도 가지고 있다.ㅋㅋ
    }
}
